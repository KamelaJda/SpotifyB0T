        ...
        () -> {
          ...
          if (c != null && c.getCommandData() != null) {
            ...
            try {
              ...
              EmbedBuilder embed =
                  e.isFromGuild()
                      ? HelpCommand.embed(c, e.getMember(), Static.defualtLanguage)
                      : HelpCommand.embed(c, Static.defualtLanguage);
              context.getHook().sendMessageEmbeds(embed.build()).complete();
            } catch (Exception ex) {
              ex.printStackTrace();
              context
                  .getHook()
                  .sendMessage(Static.defualtLanguage.get("global.command.error"))
                  .complete();
            }
          }
        };
